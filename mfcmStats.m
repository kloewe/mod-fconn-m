function varargout = mfcmStats(fun,data,varargin)
%MFCMSTATS Element-wise statistics across multiple functional connectomes.
%
%   A = MFCMSTATS(FUN1,F,...) computes an N-by-N matrix of connection-level
...
%     FUN1            Description
%     ----            -----------
%     'sum'           sum                      A = MFCMSTATS('sum',F)
%
%     'mean'          sample mean              A = MFCMSTATS('mean',F)
%
%     'var'           sample variance          A = MFCMSTATS('var',F)
%
%     'std'           sample standard dev.     A = MFCMSTATS('std',F)
%
%     'tstat'         t statistic              A = MFCMSTATS('tstat',F)
%                     (one-sample)
%
%                     [A,P] = MFCMSTATS('tstat',F) also returns the
%                     corresponding p values in P.
%
%   A = MFCMSTATS(FUN2,F,G,...) can be used to compute statistics based on
...
%
%   A = MFCMSTATS(FUN2,F1,F2) can be used to pass the data sets forming
...
%     FUN2            Description
%     ----            -----------
%     'mdiff'         mean differences         A = MFCMSTATS('mdiff',F,G)
%                                              A = MFCMSTATS('mdiff',F1,F2)
%
%     'tstat2'        t statistic              A = MFCMSTATS('tstat2',F,G)
%                     (two indep. samples,     A = MFCMSTATS('tstat2',F1,F2)
%                      equal variances)
%
...
%
%                     [A,P] = MFCMSTATS('tstat2',...) also returns the
%                     corresponding p values in P.
%
%     'pairedt'       t statistics             A = MFCMSTATS('pairedt',F,G)
%                     (two dependent/paired    A = MFCMSTATS('pairedt',F1,F2)
%                      samples)
%
%   A = MFCMSTATS('corrv',F,V) can be used to compute for each connection the
%   Pearson correlation between the connectivity values from the individual
%   connectomes and the specified variable V. F is an T-by-N-by-S array
%   containing S functional data sets. V is a numeric vector of length S.
%
%   A = MFCMSTATS('didt',FA1,FA2,FB1,FB2,...) computes a difference-in-
